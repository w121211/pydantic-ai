Pydantic-AI is model-agnostic and has built in support for the following providers:
...
To use each model provider, you need to ensure you have the appropriate packages installed and configure your local environment with the right API keys.

## Installation

To use the models, you need to either install [`pydantic-ai`](install.md), or install [`pydantic-ai-slim`](install.md#slim-install) with the appropriate optional group:

```bash
pip/uv-add 'pydantic-ai-slim[provider]'
```

Replace `provider` with the appropriate model provider name (e.g., `openai`, `anthropic`, `gemini`, `groq`, `mistral`).

You can also use [`pydantic-ai-slim`](install.md#slim-install) with multiple optional groups. For example:

```bash
pip/uv-add 'pydantic-ai-slim[openai,anthropic,gemini]'
```

## Configuration

To use models, you need to provide an API key for the respective model provider.
...
Once you have the API key for the respective model provider, you can set it as an environment variable:
...
You can use your model of choice by creating an instance of the [`Agent`][pydantic_ai.agent.Agent] class with the model name
as the first argument:

```python {title="model_by_name.py"}
from pydantic_ai import Agent
...
Or by creating an instance of the model class and passing it to the [`Agent`][pydantic_ai.agent.Agent] class:

```python {title="model_init.py"}
from pydantic_ai import Agent
from pydantic_ai.models.openai import OpenAIModel
